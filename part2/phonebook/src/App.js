		...
		if (person) {
			personService
				...
				.then(returnedPerson => {
					...
					successAnnouncement(`Updated information of ${personObject.name}`)
				})
				.catch(() => {
					errorAnnouncement(`Information of ${personObject.name} has already been removed from server`)
				})
		} else {
			personService
				.create(personObject)
			...
			successAnnouncement(`Added information of ${personObject.name}`)
		}
